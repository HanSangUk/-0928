...
<c:forEach var="taglist" items="${taglist}" >
	<div class="row1">
 		<div class="column1">
      			<a href="boardview?bnumber=${taglist.bnumber}"><img src="File/${taglist.bfile}" alt="Jane" style="width:100%" id="star"></a>
        		<h2><a href="boardview?bnumber=${taglist.bnumber}&page=${tagserch.page}">${taglist.btitle}</a></h2>
        		<p class="title">${taglist.bwriter}</p>
        		<c:if test="${not empty taglist.btag1}">
        		<button id="tag1" onclick="tagview('${taglist.btag1}')">#${taglist.btag1}</button>
        		</c:if>
        		<c:if test="${not empty taglist.btag2}">
        		<button id="tag2" onclick="tagview('${taglist.btag2}')">#${taglist.btag2}</button>
  				</c:if>
  		</div>
	</div>
</c:forEach> 
	...
	<!-- 페이징 처리 -->
	<c:if test="${tagserch.page<=1}">
	[이전]&nbsp;
	</c:if>
	
	<c:if test="${tagserch.page>1}">
		<a href="boardtag?page=${tagserch.page-1}">[이전]</a>&nbsp;
	</c:if>
	
	<c:forEach begin="${tagserch.startPage}" end="${tagserch.endPage}" var="i" step="1">
		<c:choose> 
			<c:when test="${i eq tagserch.page}">
			${i}
			</c:when>
			<c:otherwise>
				<a href="boardtag?page=${i}">${i}</a>
			</c:otherwise>
		</c:choose>
	</c:forEach>
	<c:if test="${tagserch.page>=tagserch.maxPage}">
		[다음]
	</c:if>
	<c:if test="${tagserch.page<tagserch.maxPage}">
		<a href="boardtag?page=${tagserch.page+1}">[다음]</a>
	</c:if><br>
	...
	<form action="searchs" name="searchform" class="search">
		<select name="kategorie" style="HEIGHT: 40px;">
